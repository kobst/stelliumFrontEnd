...
const generateSynastryChartDescription = async (synastryAspects, birthchartA, birthchartB, userAName, userBName) => {
    ...
    const planetDescriptionsA = birthchartA.planets.map(planet => {
        ...
        return `${planet.name} in ${userBName}'s chart is in the ${house}th house`;
      });
    ...
    const planetDescriptionsB = birthchartB.planets.map(planet => {
        ...
        return `${planet.name} in ${userAName}'s chart is in the ${house}th house`;
    });
...
const generateCompositeChartPlanetDescriptions = async (compositeChart) => {
  ...
  const fetchCompositeChartInterpretation = async (compositeChartId, compositeChartDescription, compositeChartPlanetDescriptions) => {
    ...
    try {
      ...
      console.log('fetchedInterpretation Composite Chart');
  ...
  const saveInterpretation = async (heading, promptDescription, interpretation, isCompositeChart = false) => {

    const savedInterpretation = await saveCompositeChartInterpretation(compositeChart._id, heading, promptDescription, interpretation, isCompositeChart);
    console.log("savedInterpretation: ", savedInterpretation)
  }
                ...
                {Object.entries(compositeChartPromptDescriptionsMap).map(([heading, promptDescription], index) => (
                  <tr key={index}>
                    ...
                    <td>
                      ...
                      <button 
                        onClick={() => saveInterpretation(heading, promptDescription, compositeChartHeadingInterpretationMap[heading], true)}
                        disabled={!compositeChartHeadingInterpretationMap[heading]}
                      >
                        Save Interpretation
                      </button>
                        ...
                        {Object.entries(synastryPromptDescriptionsMap).map(([heading, promptDescription], index) => (
                            <tr key={index}>
                                ...
                                <td>
                                    ...
                                    <button 
                                        onClick={() => saveInterpretation(heading, promptDescription, synastryHeadingInterpretationMap[heading], false)}
                                        disabled={!synastryHeadingInterpretationMap[heading]}
                                    >
                                        Save Interpretation
                                    </button>
