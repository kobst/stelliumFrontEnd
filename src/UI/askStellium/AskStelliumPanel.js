...
function AskStelliumPanel({
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      ...
      try {
        const history = await (fetchHistoryOverride || config.fetchHistory)(contentId, 50);
    ...
    loadHistory();
  }, [isOpen, contentId, config, fetchHistoryOverride]);
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      ...
      if (sendMessageOverride) {
        response = await sendMessageOverride({
          contentType,
          contentId,
          message: userMessage,
          selectedElements,
        ...
        responseText = response?.answer || response?.response || response?.text || '';
      } else if (contentType === 'birthchart' || contentType === 'analysis') {
        const requestBody = {};
        if (userMessage) {
